const __static = "C:\\Users\\timmi\\PriceUI\\static"
...
process.env["NODE_CONFIG_DIR"] = __static + '/config';
...
const config = require('config') //работа с конфигурационным файлом (для настройки)
const emlformat = require('eml-format'); //Для работы с eml
const MAIN_PATH = config.get('Основное.ПутьКОсновномуШаблону');
const CLIENTS = config.get('Основное.ПолучателиПрайсов');
const notifier = require('mail-notifier');
const lodash = require('lodash')
const _ = require('lodash'); //работа с объектами и массивами
let emitter = undefined; //пишет в консоль vue


let imapCounter = 0;
const imap = {
  user: config.get('ПочтаСПрайсами.ИмяПользователя'),
  password: config.get('ПочтаСПрайсами.Пароль'),
  host: config.get('ПочтаСПрайсами.Хост'),
  port: config.get('ПочтаСПрайсами.Порт'),
  tls: config.get('ПочтаСПрайсами.Шифрование'),
...
const notify = notifier(imap);

let mailTransport;

mailTransport = nodemailer.createTransport({
  host: config.get('ПочтаДляРассылки.ХостингПочты'),
  port: config.get('ПочтаДляРассылки.ПортПочты'),
  secure: config.get('ПочтаДляРассылки.ШифрованиеПочты'), // true for 465, false for other ports 587
  auth: {
    user: config.get('ПочтаДляРассылки.ИмяПользователя'),
    pass: config.get('ПочтаДляРассылки.Пароль')
  }
});
...
async function writeMail(path, data, date, template, index) {
  return new Promise(async resolve => {
    if (data !== undefined) fs.writeFileSync(path, data);
    template[0].data[index][0] = path;
    template[0].data[index][6] = Date.parse(date);
    console.log('Найден новый прайс ' + template[0].data[index][5] + ' на ' + date.toLocaleDateString())
    resolve(template)
  });
}
  ...
  return new Promise(resolve => {
    ...
    notify
        ...
        .on('mail', function (mail) {
          ...
          template[0].data.map(async (elem) => {

            if (mail.attachments !== undefined && elem[3] !== undefined) {

              if (elem[3].toLowerCase() === mail.from[0].address.toLowerCase() && (elem[6] === undefined ? true : elem[6] < Date.parse(mail.date))) {

                mail.attachments.forEach(attach => {
                  fs.writeFileSync(__static + config.get('Основное.ПодпапкаСВложениями') + attach.fileName, attach.content)
                  attach.path = __static + config.get('Основное.ПодпапкаСВложениями') + attach.fileName
                })

                if (elem[7] !== undefined) {
                  if (!mail.subject.includes(elem[7])) {
                    return;
                  }
                ...
                if (elem[4] === undefined) {
                  ...
                  if (mailPath.substring(mailPath.lastIndexOf('.') + 1, mailPath.length) === 'eml') {
                    ...
                    emlformat.read(eml, async (error, data) => {
                      ...
                      template = await writeMail(writePath, data.attachments["0"].data, mail.date, template, template[0].data.indexOf(elem));
                    ...
                    if (mailPath.substring(mailPath.lastIndexOf('\\') + 1, mailPath.lastIndexOf('.')) === 'price') {
                      ...
                      let newName = mailPath.substring(0, mailPath.lastIndexOf('\\') + 1) + elem[5] + mailPath.substring(mailPath.lastIndexOf('.'), mailPath.length)
                      template = await writeMail(newName, buffer, mail.date, template, template[0].data.indexOf(elem))

                    } else {
                      template = await writeMail(mailPath, undefined, mail.date, template, template[0].data.indexOf(elem))
                    }
                  }
                } else {
                  //Если стоит : в шаблоне, значит это архив, работа с архивом
                  if (elem[4].includes(':')) {
                    let elemZip = elem[4].substring(0, elem[4].indexOf(':'));
                    let elemFilter = elem[4].substring(elem[4].indexOf(':') + 1, elem[4].length);
                        ...
                        files.forEach((file) => {
                          zip.files[file].async('uint8array').then(async (uint8array) => {
                            if (file.includes(elemFilter)) {
                              template = await writeMail(folder + '\\' + file, uint8array, mail.date, template, template[0].data.indexOf(elem))
                            }
                          })
                    ...
                    let filteredAttach = mail.attachments.filter((elem2) => elem2.path.includes(elem[4]));


                    if (filteredAttach.length > 0) {
                      template = await writeMail(filteredAttach[0].path, undefined, mail.date, template, template[0].data.indexOf(elem))
                    }
                  }
                }
              }
