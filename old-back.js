...
function mergeArrays(a1, a2, propLeft, propRight, newLeft, newRight, addFilter, addFilterColumn, after) {
  let skip = false;
  if (!addFilter || !addFilterColumn) {
    skip = true;
  }
  ...
  for (let i = 1; i < a1.length; i++) { 
    if (skip || eval(`${a1[i][addFilterColumn]}${addFilter}`)) {
      ...
      if (~filterIndex) {
        for (let j = 0; j < newLeft.length; j++) {
          if (after) {
	  if (parseFloat(filteredAnswer[filterIndex][newRight[j]].replace(/ /g,'').replace(/,/g,'.')) == filteredAnswer[filterIndex][newRight[j]].replace(/ /g,'').replace(/,/g,'.')) {
	      filteredAnswer[filterIndex][newRight[j]] = parseFloat(filteredAnswer[filterIndex][newRight[j]].replace(/ /g,'').replace(/,/g,'.'));
	  }
          }
...
async function main() {
  ...
  let query = `SELECT convert_rules.id,
			list_name,
                        sender,
                        convert_rules.id || '_' || t.pseudoname  AS outer_name,
                        filter,
          ...
          await Promise.all(template.map(async (elem) => {
            return new Promise(async resolve1 => {
              if (mail.attachments !== undefined && elem.sender !== null
                  && elem.sender.toLowerCase() === mail.from[0].address.toLowerCase()) {
...
//if (template[i].outer_name === 'CS CJD') {
...
async function convertFiles(path, template, tables, resultName, listName) {
  return new Promise(async resolve => {
    ...
    if (path.substring(path.length - 3, path.length) === 'txt') {
      ...
      let ee = await modifyExcel(object, template, arrayOfTables);
...
async function modifyExcel(parsedObject, template, arrayOfTables, resultName) {
  return new Promise(async (resolve) => {
	    ...
	    if (templateElement.includes('AFTER')) {
		continue;
	    }
            ...
            let newColumn = templateElement.substring(templateElement.indexOf('|') + 1, templateElement.indexOf('(') === -1 ? templateElement.length : templateElement.indexOf('('));
            //A-0 B-1 C-2 ...
            ...
            let addFilter;
            let addFilterColumn;
            if (templateElement.indexOf('(') !== -1) {
              addFilterColumn = ('ABCDEFGHIJKLMNOPQRSTUVWXYZ').indexOf(templateElement.substring(templateElement.indexOf('(') + 1, templateElement.indexOf(';')));
              addFilter = templateElement.substring(templateElement.indexOf(';') + 1, templateElement.indexOf(')'));
            }
            mergeArrays(parsedObject[0].data, filtered.rows, accordLeft, accordRight, newColumnLeft, newColumnRight, addFilter, addFilterColumn, false)
	...
	//Объединяем таблицы так, как это описано в таблице шаблонов
		...
		templateElement = templateElement.replace(/AFTER/g, '');
	    }
            ...
            mergeArrays(newExcel, filtered.rows, accordLeft, accordRight, newColumnLeft, newColumnRight, addFilter, addFilterColumn, true)
